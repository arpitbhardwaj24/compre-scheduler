  ...
  return (
    <div>
      <select
        style={{ width: "70%" }}
        onChange={(e) =>
          updateInvigilator(row, col, data, index, e.target.value)
        }
      >
      ...
      <button
        ...
        onClick={() => unallotInvigilator(row, col, data, index)}
      >
        X
      </button>
    </div>
...
export default function Block({
  ...
  return (
    ...
    >
      ...
      {instructorOpen ? (
        ...
        >
          {data.instructors?.map((el) => {
            return (
              <div style={{ padding: 2, marginBottom: 2, marginTop: 2 }}>
                {el}
              </div>
            );
          })}
      ...
      >
        Invigilators ({data.allotedInvigilators?.length}){" "}
        {invigilatorOpen ? "▲" : "▼"}
      </div>
